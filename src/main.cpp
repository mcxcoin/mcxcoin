...
const string strMessageMagic = "mcxcoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
            ...
            int64_t nSubsidy = 1 * COIN; //Miner reward

            if(nBestHeight <= 800)
            {
            nSubsidy = 10000 * COIN; //Total premine coin
            }
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    ...
    printf("Block Number %d \n",nBestHeight);
        ...
        if(nBestHeight < YEARLY_BLOCKCOUNT)
    	    nRewardCoinYear = 4.8 * COIN_YEAR_REWARD; //48% reward for 131400 blocks
    	else if(nBestHeight < (2 * YEARLY_BLOCKCOUNT))
    	    nRewardCoinYear = 4.2 * COIN_YEAR_REWARD; //42% reward for 131400 * 2 blocks
    	else if(nBestHeight < (3 * YEARLY_BLOCKCOUNT))
    	    nRewardCoinYear = 3.6 * COIN_YEAR_REWARD; //36% reward for 131400 * 3 blocks
            ...
            nRewardCoinYear = 1.8 * COIN_YEAR_REWARD; //18% reward for 131400 * 5 blocks
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "mcxcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "27dec2017";
        CTransaction txNew;
        txNew.nTime = 1514383644;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1514383644;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 1689878 : 1689878;
...
//        assert(block.hashMerkleRoot == uint256("0x133d1cc089d45a17d6708837432634d9edd1f88f18ad827eb5f0a83f0c93fdde"));
        assert(block.hashMerkleRoot == uint256("0x5acf3d69bc448fa21ad9f69342d05cb311fc334e17068c24616e0ad84c36bb3e"));
...
unsigned char pchMessageStart[4] = { 0xc3, 0xbd, 0xd2, 0xd1 };
